...
export class ActorRuntime {
  ...
  update(dt: number, distanceToPlayer: number, targeted: boolean): void {
    this.machine.blackboard.patch({ distanceToPlayer, targeted });

    const state = this.machine.getCurrentStateId();
    if (state === ACTOR_STATES.disabled || state === ACTOR_STATES.hidden) {
    ...
    if (targeted && state === ACTOR_STATES.idle) {
      ...
      this.machine.request(ACTOR_STATES.targeted, 'player-targeted');
  ...
  beginInteraction(
    mode: 'talking' | 'trading' | 'travelling' | 'performing',
  ): boolean {
    const state = this.machine.getCurrentStateId();
    if (state === ACTOR_STATES.disabled || state === ACTOR_STATES.hidden) {
      return false;
  ...
  finishInteraction(): void {
    this.machine.blackboard.set('goal', 'Waiting');
    this.machine.request(ACTOR_STATES.idle, 'interaction-completed');
  }
