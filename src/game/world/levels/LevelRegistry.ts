...
export class LevelRegistry {
  ...
  validate(): readonly string[] {
    ...
    for (const level of this.levels.values()) {
      ...
      for (const transition of level.transitions) if (!zoneIds.has(transition.zoneId)) issues.push(`${level.id}: transition ${transition.id} references missing zone ${transition.zoneId}`);
    }
    return issues;
  }
}
