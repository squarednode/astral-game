...
export class SkillTreeScreen {
  ...
  setOpen(open: boolean): void { this.host.classList.toggle('hidden', !open); }
  ...
  dispose(): void { this.host.removeEventListener('click', this.onClick); }
    ...
    this.host.innerHTML = `
      <div class="skill-tree-shell">
        <header class="skill-tree-header">
          ...
          <button data-action="close" aria-label="Close skill tree">×</button>
        </header>
        <nav class="skill-character-list">
          ${this.characters.map(character => `<button class="${character.id === selected?.id ? 'selected' : ''}" data-action="select" data-character-id="${character.id}"><strong>${character.name}</strong><span>${character.tree.identityTitle}</span><small>Level ${character.state.level} · ${character.state.availableSkillPoints} point${character.state.availableSkillPoints === 1 ? '' : 's'} · ${character.rosterStatus}</small></button>`).join('')}
        </nav>
    ...
    return `<main class="skill-tree-main">
      ...
      <section class="skill-node-grid">
        ${tree.nodes.map(node => {
          ...
          return `<article class="skill-node ${status} branch-${node.branch}"><div class="skill-node-tier">Tier ${node.tier} · ${node.branch}</div><h3>${node.name}</h3><p>${node.description}</p><small>${requirement}</small>${unlocked ? '<b>✓ Learned</b>' : `<button data-action="unlock" data-character-id="${character.id}" data-node-id="${node.id}" ${available ? '' : 'disabled'}>Unlock · ${node.cost}</button>`}</article>`;
        }).join('')}
      </section>
    </main>`;
  }
